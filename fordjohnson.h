...
class fj {
	private:	
		...
		void printvec(C &vec, int J=1);
		template<typename T, class C = std::vector<T>>
		void swap(C &vec, int a, int b);
		template<typename T, class C = std::vector<T>>
		void lcomp(C &vec, int b, long J=1);
		template<typename T, class C = std::vector<T>>
		int binarySearch(C &vec, int i, int j, int J);
		template<typename T, class C = std::vector<T>>
		void shift(C &vec, int a, int i, int J);
	public:
		template<typename T, class C = std::vector<T>>
		void sort(C &vec, long J=1);
};
