...
should generate \ttt{cmov}s and the one that should generate jumps. The time
...
\ttt{valgrind} and \ttt{cachegrind} in Figure \ref{fig:branch-predict} we see
why.
...
\tit{OpenMP} to parallelise the second loop of the implementation, there are
...
\begin{figure}
    ...
    \end{tabular}
    \caption{Comparison of \ttt{std::sort} to the different versions of Bitonic
...
mergesort to \ttt{std::sort()}. We clearly see that \ttt{std::sort()} wins by
